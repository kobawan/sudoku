 ...
 */
export const sortByRows = <ArrayType = HTMLTextAreaElement>(game: Game, callback: (values: RowValues<ArrayType>) => void) => {
    let pos: number, values: RowValues<ArrayType>, arr: ArrayType[][] = [];
 ...
 * Returns array from callback.
 */
export const sortByCols = <ArrayType = HTMLTextAreaElement>(game: Game, callback: (values: ColumnValues<ArrayType>) => void) => {
    let pos: number, values: ColumnValues<ArrayType>, arr: ArrayType[][] = [];
    for (let col = 0; col < game.gameType; col++) {
        arr.push([]);
        for (let row = 0; row < game.gameType; row++) {
 ...
 * Returns array from callback.
 */
export const sortByGrids = <ArrayType = HTMLTextAreaElement>(game: Game, callback: (values: GridValues<ArrayType>) => void) => {
    let grid: number, rowPos: number, colPos: number, pos: number, values: GridValues<ArrayType>, arr: ArrayType[][] = [];
