...
// THUNKS
const initLoggedInUserInfo =
  (user: UserData): AppThunk =>
  (dispatch) => {
...
const initGuestUserInfo =
  ({ config, state }: SavedGame): AppThunk =>
  (dispatch) => {
    if (config && state) {
...
export const findUser = (): AppThunk => async (dispatch) => {
  ...
  try {
    ...
    dispatch(setError(error as Error | RequestError));
...
export const loginUser =
  (username: string, password: string, isNewUser: boolean): AppThunk =>
  async (dispatch) => {
    try {
      ...
      const err = error as Error | RequestError;
      if ((err as RequestError).isValidationError) {
